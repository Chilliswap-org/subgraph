...
import { updatePairDayData, updateTokenDayData, updatedifxDayData, updatePairHourData } from './dayUpdates'
...
export function handleTransfer(event: Transfer): void {
  ...
  let factory = difxFactory.load(FACTORY_ADDRESS)
...
export function handleSwap(event: Swap): void {
  ...
  let difxDayData = updatedifxDayData(event)
  let token0DayData = updateTokenDayData(token0 as Token, event)
  let token1DayData = updateTokenDayData(token1 as Token, event)

  // swap specific updating
  difxDayData.dailyVolumeUSD = difxDayData.dailyVolumeUSD.plus(trackedAmountUSD)
  difxDayData.dailyVolumeETH = difxDayData.dailyVolumeETH.plus(trackedAmountETH)
  difxDayData.dailyVolumeUntracked = difxDayData.dailyVolumeUntracked.plus(derivedAmountUSD)
  difxDayData.save()
